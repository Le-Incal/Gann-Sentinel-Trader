...
- Technical signals tracking
- Chart analysis section in scan summaries
- Technical context in /check results
...
EMOJI_CHART_DOWN = "\U0001F4C9"  # 📉 (NEW)
...
EMOJI_YELLOW_CIRCLE = "\U0001F7E1" # 🟡 (NEW)
...
EMOJI_ZZZ = "\U0001F4A4"         # 💤
EMOJI_CANDLE = "\U0001F56F"      # 🕯️ (NEW - for chart analysis)
EMOJI_RULER = "\U0001F4CF"       # 📏 (NEW - for support/resistance)
...
class TelegramBot:
    ...
    def __init__(
        ...
        self._technical_signals: List[Dict[str, Any]] = []  # NEW
    ...
    async def send_scan_summary(
        ...
        technical_signals: Optional[List[Dict[str, Any]]] = None  # NEW
    ) -> bool:
        ...
        if tech_signals:
            ...
            for tech in tech_signals[:3]:  # Show top 3
        ...
        if signals:
            ...
            for sig in signals:
                ...
                # Skip technical signals (shown above)
    ...
    async def send_check_result(self, result: Dict[str, Any]) -> bool:
        ...
        technical = result.get("technical_analysis")  # NEW
    ...
    async def send_daily_digest(
        ...
        lines.append(f"\n{EMOJI_KEYBOARD} /status /pending /check [TICKER] /help")
        lines.append("=" * 40)
