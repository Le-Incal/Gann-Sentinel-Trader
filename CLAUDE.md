...
- When you create or modify files in this repo, **always** commit and push at the end of your response (or after a logical set of changes). No exceptions.
  - `git add -A && git commit -m "<short descriptive message>" && git push`
  - Use `git push` (or `git push origin main`) so changes are on the remote.
- Do **not** say "I've made the changes; you can commit when ready" or ask the user to commit. Just commit and push automatically.
